...
#ifdef HAVE_DISCORDRPC
	{IT_STRING | IT_SUBMENU,  NULL, "\x80" "Discord Requests...",     &MISC_DiscordRequestsDef, 40},
#endif
...
void M_RefreshPauseMenu(void)
{
	...
	else
	{
		...
		MPauseMenu[mpause_discordrequests].text = "\x80" "Discord requests...";
	}
#endif
}
...
static void M_DrawConnectMenu(void)
{
	...
	for (i = 0; i < min(serverlistcount - serverlistpage * SERVERS_PER_PAGE, SERVERS_PER_PAGE); i++)
	{
		...
		if (serverlist[slindex].info.refusereason == REFUSE_BANNED) // probably banned or something. Probably.
			V_DrawSmallString(currentMenu->x+270, S_LINEY(i)+4, globalflags &~(V_TRANSLUCENT|MENUHIGHLIGHT), "\x85" "X");
