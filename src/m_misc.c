...
void M_FirstLoadConfig(void)
{
	...
	// load config again except for srb2-edit client cvars
	COM_BufInsertText(va("exec \"%s\"\n", editconfigfile));
...
/** Saves the game configuration.
  ...
  */
void M_SaveConfig(const char *filename, boolean foredit)
{
#define THISCONFIG (foredit ? editconfigfile : configfile)
	FILE *f;
	char *filepath;
	...
	{
		if (!strstr(THISCONFIG, ".cfg"))
		{
			CONS_Alert(CONS_NOTICE, M_GetText("Config filename must be .cfg\n"));
			return;
		}

		f = fopen(THISCONFIG, "w");
		if (!f)
		{
			CONS_Alert(CONS_ERROR, M_GetText("Couldn't save game config file %s\n"), THISCONFIG);
			return;
	...
	if ((tutorialmode && tutorialgcs) && !foredit)
	{
		...
		CV_SaveVariables(f, foredit);

	if (!dedicated)
	{
		if (tutorialmode && tutorialgcs)
			G_SaveKeySetting(f, gamecontroldefault[gcs_custom], gamecontrolbis, foredit); // using gcs_custom as temp storage
		else
			G_SaveKeySetting(f, gamecontrol, gamecontrolbis, foredit);
	}
...
#define SRB2PNGTXT 11 //PNG_KEYWORD_MAX_LENGTH(79) is the max
	...
	char titletxt[] = "Sonic Robo Blast 2 edit " VERSIONSTRING;
